...
public class Graph {
    ...
    // The vertices in teh graph. Every vertex must have a unique String label.
     ...
     * to all other veritces in this Graph. 
     ...
     * to all other veritces in this Graph using Dijkstra's algorithm. 
     * 
    ...
    public void dijkstra(String startName) {
        ...
        vertices.get(currentStartVertexName).weightedCostFromStartVertex=0;
        vertices.get(currentStartVertexName).numEdgesFromStartNode=0;
        paths.add(new Path(vertices.get(currentStartVertexName), 0));
        while(!paths.isEmpty()){
        	Path temp = paths.remove();
        	Vertex curr = temp.dest;
        	if (curr.scratch != 1){
        		curr.scratch = 1;
        		for (Edge e:curr.adjacent){
        			Vertex vert = e.dest;
        			if (vert.weightedCostFromStartVertex > curr.weightedCostFromStartVertex+e.cost){
        				vert.prev = curr; 
        				vert.weightedCostFromStartVertex = curr.weightedCostFromStartVertex+e.cost;
        				vert.numEdgesFromStartNode = curr.numEdgesFromStartNode+1;
        				paths.add(new Path(vert, vert.weightedCostFromStartVertex));
        			}
        		}
        	}	
        }
     ...
     * the shortest paths from a vertex to the vertices it 
     ...
     * connects to.
     * <br><br>Note, if the parameter weighted is false, 2 and 3 must
     * equal each other.
     * 
     * <br><br>This method also determines the path with the greatest distance.
     * The two most distant vertices in the Graph are those with the
     * lognest shortest path between them. The longest path is based on
    ...
    public void findAllPaths(boolean weighted) {
        ...
        if (weighted){
        	for (String s : vertices.keySet()){
        		Vertex curr = vertices.get(s);
        		curr.clearPathInfo();
        		clearAll();
        		dijkstra(s);
        		for (Vertex v: vertices.values()){
        			if (v.weightedCostFromStartVertex <INFINITY && !v.equals(curr)){
        				curr.totalUnweightedPathLengh += v.numEdgesFromStartNode;
        				curr.totalWeightedPathLength += v.weightedCostFromStartVertex;
        				curr.numVertexConnected++;
        				if (v.weightedCostFromStartVertex>longest.weightedCostOfPath){
        					longest = getPath(v.name);
        				}
        			}
        		}
        	}
        	...
        	for (String s : vertices.keySet()){
        		...
        		curr.clearPathInfo();
        		clearAll();
        		findUnweightedShortestPath(s);
        		for (Vertex v: vertices.values()){
        			if (v.weightedCostFromStartVertex <INFINITY && !v.equals(curr)){
        				curr.totalUnweightedPathLengh += v.numEdgesFromStartNode;
        				curr.totalWeightedPathLength += v.weightedCostFromStartVertex;
        				curr.numVertexConnected++;
        				if (v.numEdgesFromStartNode>longest.weightedCostOfPath){
        					longest = getPath(v.name);
        				}
        			}
        		}
    ...
    private static class Vertex {
        ...
        // If an edge alreadt exists replace the previous
