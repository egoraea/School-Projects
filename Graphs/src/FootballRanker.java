...
public class FootballRanker {
    ...
    private double printRootMeanSquareError(List<String> humanRanks,
        ...
        for (AllPathsInfo a : paths){
        	predicted.add(a.getName());
        }
        for (int i = 0; i<humanRanks.size();i++){
        	String s = humanRanks.get(i);
        	result +=  ((i+1)-(predicted.indexOf(s)+1))*(((i+1)-(predicted.indexOf(s)+1)));
        	if (showResults)
        		System.out.println(String.format("%1$-" + PADDING  + "s", s)+"- actual rank: "+(i+1)+" predicted rank: "+(predicted.indexOf(s)+1));
        }
        result = result/humanRanks.size();
