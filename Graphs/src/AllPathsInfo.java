...
/**
 * Stores information about a vertex and its paths
 * to other vertices. Paths are not stored individually. Instead aggregate
 * statistics for the paths from a given vertex
 * are stored including the name of the vertex, 
 * the number of other vertices the starting vertex connects to,
...
public class AllPathsInfo implements Comparable<AllPathsInfo>{
    ...
    private int numVerticesConnected;
    ...
    public AllPathsInfo(String st, int ps, double total) {
        ...
        vertexName = st;
        numVerticesConnected = ps;
        totalCost = total;
        aveCost = totalCost / numVerticesConnected;
    }

    /**
     * Primary key is number of vertices connected, then average cost per path.
     */
    public int compareTo(AllPathsInfo other) {
        int result = other.numVerticesConnected - numVerticesConnected;
        if(result == 0)
    ...
    public String toString() {
        return "Name: " + String.format("%1$-" + PADDING  + "s", vertexName) + " cost per path: " + ourFormatter.format(aveCost) + ", num paths: " + numVerticesConnected;
    }
    ...
    public static AllPathsInfo makeCopy(AllPathsInfo org) {
        AllPathsInfo result = new AllPathsInfo(org.vertexName, org.numVerticesConnected, org.totalCost);
        return result;
    }
